...
def save_logs_t_leNet(output_directory, hist, y_pred, y_true, duration, fold = ''):
    hist_df = pd.DataFrame(hist.history)
    hist_df.to_csv(os.path.join(output_directory, f'{fold}_history.csv'), index=False)

    df_metrics = calculate_metrics(y_true, y_pred, duration)
    df_metrics.to_csv(os.path.join(output_directory, f'{fold}_df_metrics.csv'), index=False)
    ...
    df_best_model.to_csv(os.path.join(output_directory, f'{fold}_df_best_model.csv'), index=False)

    # plot losses
    plot_epochs_metric(hist, os.path.join(output_directory, f'{fold}_epochs_loss.png'))


def save_logs(output_directory, hist, y_pred, y_true, duration, lr=True, y_true_val=None, y_pred_val=None, fold = ''):
